...
- Updates Inventory layout positions/sizes requested by user.
- Mirrors SideKicks card / info / divider visual language for Inventory Skins.
- Keeps Inventory-specific content and equip button.
- Creates backups under _local_backups/inventory_sidekick_theme_v2/

Run from repo root:
    python apply_inventory_sidekick_theme_v2.py
"""
...
INV = BASE / "Menus" / "Inventory"
BACKUP = ROOT / "_local_backups" / "inventory_sidekick_theme_v2"
...
def main() -> None:
    if not INV.exists():
        raise SystemExit(f"Inventory folder not found: {INV}")

    write(INV / "init.lua", INVENTORY_INIT)
    write(INV / "SkinsPage.lua", SKINS_PAGE)
    write(INV / "SkinCard.lua", SKIN_CARD)
    write(INV / "SelectedSkinInfo.lua", SELECTED_SKIN_INFO)

    print()
    print("Inventory SideKicks-theme v2 applied.")
