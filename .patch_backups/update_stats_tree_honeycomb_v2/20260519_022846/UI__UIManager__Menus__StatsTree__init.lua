...
local function nodeView(node: NodeDefinition, levels: Source<{ [string]: number }>, opened: Source<{ [string]: boolean }>, selectedId: Source<string?>, points: Source<number>, onClick: (NodeDefinition) -> (), z: number)
	...
	return create("ImageButton")({
		...
		Visible = function() return isVisible(node, opened(), levels()) end, Activated = function() onClick(node) end,
...
local function StatsTreeMenu(rawProps: StatsTreeMenuProps?) local props: StatsTreeMenuProps = rawProps or {}
	...
	local function menuVisible(): boolean local visible = props.visible if visible ~= nil then return visible() end local store = props.store if store ~= nil then local currentMenu = store.currentMenu if currentMenu ~= nil then return currentMenu() == "StatsTree" end end return true end
	...
	for _, n in ipairs(Data.nodes) do
		table.insert(canvasChildren, nodeView(n, levels, opened, selectedId, points, clickNode, 110))
	end
	...
	return create("Frame")({
		...
		create("TextButton")({
			...
			Activated = function() local store = props.store if store == nil then return end local currentMenu = store.currentMenu if currentMenu ~= nil then currentMenu(nil) end end,
